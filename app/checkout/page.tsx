  ...
  return (
    <main className="checkout-page">
      ...
      <section className="checkout-shell">
        ...
        <aside className="order-summary"><small>ORDER SUMMARY</small><h2>{items.length} item{items.length === 1 ? "" : "s"}</h2>{items.length ? <>{items.map((item, index) => <div className="summary-item" key={`${item.id}-${index}`}><div className="summary-thumb">AQ</div><div><b>{item.name}</b><small>{item.size}</small></div><strong>₹{item.price}</strong></div>)}<div className="summary-totals"><div><span>Subtotal</span><b>₹{subtotal}</b></div><div><span>Shipping</span><b>{shipping ? `₹${shipping}` : "FREE"}</b></div><div><span>Total</span><b>₹{total}</b></div></div></> : <p>No products in cart yet.</p>}<div className="summary-trust"><span>✓ Mobile and full address</span><span>✓ Prepaid orders only</span><span>✓ Order appears in admin desk</span></div></aside>
      </section>
    </main>
  );
}
