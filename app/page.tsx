...
export default function Home() {
  ...
  return (
    <main>
      <div className="announcement"><span>FREE SHIPPING ABOVE ₹799</span><span>•</span><span>WELCOME OFFER: GLOW15</span><span>•</span><span>MADE WITH SKIN-LOVING INGREDIENTS</span></div>
      ...
      <footer><div><button className="logo logo--footer">VELORA<span>skin rituals</span></button><p>Thoughtful body care for small, beautiful moments.</p><div className="socials"><button>ig</button><button>p</button><button>f</button></div></div><div><b>SHOP</b><a href="#shop">Body wash</a><a href="#shop">Body scrub</a><a href="#shop">Face care</a><a href="#shop">Gift sets</a></div><div><b>HELP</b><a href="#">Contact us</a><a href="#">Shipping & returns</a><a href="#">FAQs</a><a href="#">Track order</a></div><div><b>ABOUT</b><a href="#story">Our story</a><a href="#story">Ingredients</a><a href="#story">Sustainability</a><a href="#">Journal</a></div><div className="footer-contact"><b>NEED HELP?</b><p>Mon–Sat, 10am–6pm IST</p><a href="mailto:hello@velora.care">hello@velora.care</a></div></footer>
      ...
      {cartOpen && <><button className="overlay" aria-label="Close bag" onClick={() => setCartOpen(false)} /><aside className="cart-drawer"><div className="drawer-head"><div><small>YOUR RITUAL</small><h2>Shopping bag ({cart.length})</h2></div><button onClick={() => setCartOpen(false)}>×</button></div>{cartProducts.length === 0 ? <div className="cart-empty"><span>□</span><h3>Your bag is waiting</h3><p>Add a little something for your skin.</p><button className="primary" onClick={() => { setCartOpen(false); scrollToShop(); }}>Start shopping</button></div> : <><div className="cart-items">{cartProducts.map((product, index) => <div className="cart-item" key={`${product.id}-${index}`}><ProductVisual product={product} /><div><small>{product.category}</small><b>{product.name}</b><p>₹{product.price}</p><button onClick={() => setCart((items) => items.filter((_, i) => i !== index))}>Remove</button></div></div>)}</div><div className="cart-summary"><div><span>Subtotal</span><b>₹{cartTotal}</b></div><small>Shipping calculated at checkout. Free above ₹799.</small><button>Proceed to checkout →</button></div></>}</aside></>}
