...
type MediaRow = RowDataPacket & { id: number; product_id: number; media_type: "image" | "video"; media_url: string; media_mime: string; alt_text: string; sort_order: number };
type ReviewRow = RowDataPacket & { id: number; product_id: number; customer_name: string; rating: number; title: string; review_text: string; media_url: string; media_mime: string; status: "pending" | "approved" | "rejected"; created_at: Date };
export type MediaUpload = { bytes: Uint8Array; mime: string; name: string };
...
const decimalMoney = (value: unknown) => {
  const amount = Math.round(Number(value) * 100) / 100;
  if (!Number.isFinite(amount) || amount < 0 || amount > 10_000_000) throw new Error("Enter a valid purchase cost");
  return amount;
};
...
const validUpload = (upload: MediaUpload) => { const mime = clean(upload.mime, 100).toLowerCase(); if (!mime.startsWith("image/") && !["video/mp4", "video/webm"].includes(mime)) throw new Error("Upload a JPG, PNG, WEBP, MP4 or WEBM file"); if (!upload.bytes.length || upload.bytes.length > 5 * 1024 * 1024) throw new Error("Each media file must be 5 MB or smaller"); return { ...upload, mime, name: clean(upload.name, 255) }; };
...
export async function publicCatalogState(includeInactive = false) {
  ...
  const [mediaRows] = await database().query<MediaRow[]>("SELECT id, product_id, media_type, media_url, media_mime, alt_text, sort_order FROM product_media ORDER BY product_id, sort_order, id");
  const [reviewRows] = await database().query<ReviewRow[]>("SELECT id, product_id, customer_name, rating, title, review_text, media_url, media_mime, status, created_at FROM reviews WHERE status = 'approved' ORDER BY created_at DESC LIMIT 200");
  return products.map((product) => { const inventory = inventoryRows.find((row) => Number(row.product_id) === product.id); const reviews = reviewRows.filter((row) => Number(row.product_id) === product.id).map((row) => ({ id: Number(row.id), customerName: row.customer_name, rating: Number(row.rating), title: row.title, text: row.review_text, mediaUrl: row.media_mime ? `/api/review-media/${row.id}` : row.media_url, mediaType: row.media_mime.startsWith("video/") || /\.(mp4|webm)(\?|$)/i.test(row.media_url) ? "video" : "image", createdAt: new Date(row.created_at).toISOString() })); const rating = reviews.length ? reviews.reduce((sum, review) => sum + review.rating, 0) / reviews.length : 0; return { productId: product.id, stock: Number(inventory?.stock ?? 0), lowStockThreshold: Number(inventory?.low_stock_threshold ?? 5), updatedAt: inventory ? new Date(inventory.updated_at).toISOString() : null, rating: Number(rating.toFixed(1)), reviewCount: reviews.length, reviews: reviews.slice(0, 8), media: mediaRows.filter((row) => Number(row.product_id) === product.id).map((row) => ({ id: Number(row.id), type: row.media_type, url: row.media_mime ? `/api/media/${row.id}` : row.media_url, alt: row.alt_text })) }; });
}
...
export async function adminCatalogState() { await ensureSchema(); const [products, catalog] = await Promise.all([listProducts(true), publicCatalogState(true)]); const [costRows] = await database().query<(RowDataPacket & { id: number; purchase_cost: number })[]>("SELECT id, purchase_cost FROM store_products ORDER BY id"); const costs = new Map(costRows.map((row) => [Number(row.id), Number(row.purchase_cost)])); const [reviewRows] = await database().query<ReviewRow[]>("SELECT id, product_id, customer_name, rating, title, review_text, media_url, media_mime, status, created_at FROM reviews ORDER BY created_at DESC LIMIT 300"); return { products: products.map((product) => ({ ...product, purchaseCost: costs.get(product.id) ?? 0 })), catalog, reviews: reviewRows.map((row) => ({ id: Number(row.id), productId: Number(row.product_id), customerName: row.customer_name, rating: Number(row.rating), title: row.title, text: row.review_text, mediaUrl: row.media_mime ? `/api/review-media/${row.id}` : row.media_url, status: row.status, createdAt: new Date(row.created_at).toISOString() })) }; }

function productValues(body: Record<string, unknown>) {
  const name = clean(body.name, 180); const category = clean(body.category, 100); const price = money(body.price); const purchaseCost = decimalMoney(body.purchaseCost);
  if (name.length < 2 || category.length < 2) throw new Error("Product name and category are required");
  const form = ["pump", "tube", "jar", "soap", "combo"].includes(clean(body.form)) ? clean(body.form) as Product["form"] : "pump";
  return { name, category, subtitle: clean(body.subtitle), price, purchaseCost, oldPrice: money(body.oldPrice, true), shade: color(body.shade, "#eaded2"), accent: color(body.accent, "#6f4436"), form, badge: clean(body.badge, 100), size: clean(body.size, 80), concern: clean(body.concern, 160), active: body.active !== false };
}
...
async function ensureMediaSlot(productId: number) { const [rows] = await database().query<(RowDataPacket & { total: number })[]>("SELECT COUNT(*) AS total FROM product_media WHERE product_id = ?", [productId]); if (Number(rows[0]?.total ?? 0) >= 6) throw new Error("Maximum 6 images/videos are allowed per product"); }
export async function addMedia(productIdValue: unknown, typeValue: unknown, urlValue: unknown, altValue: unknown) { await ensureSchema(); const productId = await validProductId(productIdValue); await ensureMediaSlot(productId); const type = String(typeValue) === "video" ? "video" : "image"; const url = validUrl(urlValue); if (!url) throw new Error("Media URL is required"); await database().execute("INSERT INTO product_media (product_id, media_type, media_url, alt_text, sort_order, created_at) VALUES (?, ?, ?, ?, 0, ?)", [productId, type, url, clean(altValue), new Date()]); }
export async function addMediaFile(productIdValue: unknown, uploadValue: MediaUpload, altValue: unknown) { await ensureSchema(); const productId = await validProductId(productIdValue); await ensureMediaSlot(productId); const upload = validUpload(uploadValue); const type = upload.mime.startsWith("video/") ? "video" : "image"; await database().execute("INSERT INTO product_media (product_id, media_type, media_url, media_blob, media_mime, file_name, alt_text, sort_order, created_at) VALUES (?, ?, '', ?, ?, ?, ?, 0, ?)", [productId, type, upload.bytes, upload.mime, upload.name, clean(altValue), new Date()]); }
...
export async function createReview(value: unknown, uploadValue?: MediaUpload) {
  ...
  const [orderRows] = await database().query<(RowDataPacket & { items_json: string })[]>("SELECT items_json FROM orders WHERE id = ? AND mobile = ? AND payment_status = 'paid' LIMIT 1", [orderId, mobile]); const purchasedItems = orderRows[0] ? JSON.parse(orderRows[0].items_json) as { id: string }[] : []; if (!purchasedItems.some((item) => Number(item.id) === productId)) throw new Error("Paid order verification failed for this product"); const upload = uploadValue ? validUpload(uploadValue) : null; await database().execute("INSERT INTO reviews (product_id, customer_name, rating, title, review_text, media_url, media_blob, media_mime, file_name, status, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending', ?)", [productId, customerName, rating, title, text, mediaUrl, upload?.bytes ?? null, upload?.mime ?? "", upload?.name ?? "", new Date()]);
}
